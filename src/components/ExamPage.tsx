...
export const ExamPage: React.FC<ExamPageProps> = ({ pageNumber, questions, showAnswers = false }) => {
  ...
  return (
    <div className="a4-sheet p-8 exam-font text-black flex flex-col justify-between select-text">
      ...
      {/* Two Column Question Layout: Stretches full vertical height, no bottom blank space */}
      <div className="grid grid-cols-2 gap-x-6 relative flex-1 min-h-0">
        ...
        {/* Left Column: Evenly distributes questions from top to bottom */}
        <div className="pr-3 flex flex-col justify-between h-full">
          {leftQuestions.map(q => (
            <QuestionCard key={q.id} question={q} showAnswers={showAnswers} />
          ))}
        </div>

        {/* Right Column: Evenly distributes questions from top to bottom */}
      ...
      {/* End of Test Booklet banner on Page 20 */}
      {isLastPage && (
      ...
      {/* Bottom Footer Line (Clean, zero rough work space) */}
